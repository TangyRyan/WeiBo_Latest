...
from spider.config import get_env_float, get_env_int, get_env_str, load_env
...
load_env()
_WS_ROOT = get_env_str("HOT_TOPICS_WS_ROOT", "") or ""
if _WS_ROOT:
    HOT_TOPICS_ROOT = from_data_relative(_WS_ROOT)
else:
    HOT_TOPICS_ROOT = HOURLY_DIR
DEFAULT_REFRESH_SECONDS = get_env_float("HOT_TOPICS_WS_REFRESH", 10.0) or 10.0
...
class HotTopicsRepository:
    ...
    def _resolve_for_date(self, date: str, hour: Optional[int]) -> Optional[SnapshotReference]:
        ...
        if hour is not None:
            ...
            if not path.exists():
                LOGGER.info("No snapshot for %s at hour %s", date, hour)
                return None
            stat = path.stat()
            return SnapshotReference(date=date, hour=hour, path=path, mtime_ns=stat.st_mtime_ns)

        return self._resolve_latest_in_dir(target_dir, date)

    def _resolve_latest(self) -> Optional[SnapshotReference]:
        if not self.root.exists():
            LOGGER.info("Hot topics root %s missing", self.root)
            return None
        candidates = sorted((p for p in self.root.iterdir() if p.is_dir()), reverse=True)
        for directory in candidates:
            ref = self._resolve_latest_in_dir(directory, directory.name)
            if ref:
                return ref
        LOGGER.info("No hot topic snapshots available under %s", self.root)
        return None

    def _resolve_latest_in_dir(self, directory: Path, date: str) -> Optional[SnapshotReference]:
        files = sorted(directory.glob("*.json"), reverse=True)
        for file_path in files:
            try:
                hour = int(file_path.stem)
            except ValueError:
                LOGGER.debug("Skipping unexpected file in %s: %s", directory, file_path.name)
                continue
            stat = file_path.stat()
            return SnapshotReference(date=date, hour=hour, path=file_path, mtime_ns=stat.st_mtime_ns)
        return None

    @staticmethod
    def _coerce_topic_list(data: Any) -> Iterable[Dict[str, Any]]:
        if isinstance(data, dict):
            payload = data.get("topics") or data.get("data")
        else:
            payload = data
        if isinstance(payload, dict):
            payload = payload.values()
        if not isinstance(payload, Iterable):
            LOGGER.warning("Snapshot data not iterable: %s", type(payload))
            return []
...
class HotTopicsWebSocketService:
            ...
            try:
                ...
                if exc.errno == errno.EADDRINUSE:
                    if auto_port:
                        ...
                        if attempts > 20:
                            LOGGER.error(
                                "Failed to bind WebSocket after trying multiple ports starting from %s",
                                port,
                            )
                            raise
                        LOGGER.warning(
    ...
    def _register_client(self, websocket: WebSocketServerProtocol, path: Optional[str]) -> ClientSubscription:
        ...
        LOGGER.info(
            "Client connected from %s with limit=%s",
...
def parse_args() -> argparse.Namespace:
    default_host = get_env_str("HOT_TOPICS_WS_HOST", "0.0.0.0") or "0.0.0.0"
    default_port = get_env_int("HOT_TOPICS_WS_PORT", 8765) or 8765
    default_refresh = get_env_float("HOT_TOPICS_WS_REFRESH", DEFAULT_REFRESH_SECONDS) or DEFAULT_REFRESH_SECONDS
    default_log = get_env_str("HOT_TOPICS_WS_LOG", "INFO") or "INFO"
