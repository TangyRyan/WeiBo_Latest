(function () {
  ...
  function plutchik(id, accountName = mockEmotionData[0].account) {
    ...
    const data = mockEmotionData.find(d => d.account === accountName) || mockEmotionData[0];
    ...
    const dataset = emotionsOrder.map((emotion, index) => ({
      emotion,
      value: Math.max(0, Math.min(1, data.emotions[emotion] ?? 0)),
      color: colors[index]
    }));
