...
function renderHotlist(items){
  ...
  items.forEach(it => { const div = create("div","hot-item");
    div.innerHTML = `<span class="rank">${it.rank}</span><span class="name">${it.name}</span><span class="hot">🔥${it.hot||0}</span>`;
    div.onclick = ()=> loadEvent(it.name); box.appendChild(div);
  });
}
