...
    // 4) 淡出并移除可能的风险带（如果存在）
    rootGroup.selectAll(".risk-grid, .region-dot-risk, .risk-labels, .vertical-grid-line") // <-- 【已更新】添加 .vertical-grid-line 清理
      .interrupt()
      .transition().duration(transitionDuration/1.5)
      .style("opacity", 0)
      .remove();
...
      labelGroup.transition().duration(transitionDuration).style("opacity",1);
      ...
      const xScaleForAxis = d3.scaleLinear().domain([0,100]).range([padding.left, padding.left + innerW]);
      const ticks = d3.range(0,101,10); // 0, 10, 20... 100
      const axisGroup = rootGroup.append("g").attr("class","x-axis").style("opacity",0);
      axisGroup.attr("transform", `translate(0,${padding.top + innerH + 10})`);
      axisGroup.call(d3.axisBottom(xScaleForAxis)
        .tickValues(ticks)
        .tickFormat((d,i) => `${i}`) // 改为 0, 1, 2... 10
      );
      axisGroup.transition().duration(transitionDuration).style("opacity",1);

      // 添加垂直分割线 
      const numVerticalLines = 9; // 9条线，形成10个区间 (1-10)
      const verticalLineSpacing = innerW / 10; // 每个区间宽度
      const verticalLineHeight = innerH; // 竖线高度与画布内部高度一致

      d3.range(1, numVerticalLines + 1).forEach(i => {
        const xPos = padding.left + i * verticalLineSpacing;
        gridGroup.append("line") // 在 gridGroup 中绘制，确保一同淡入
          .attr("class", "vertical-grid-line")
          .attr("x1", xPos).attr("x2", xPos)
          .attr("y1", padding.top).attr("y2", padding.top + verticalLineHeight) // 从顶部到画布底部
          .attr("stroke", "rgba(16, 4, 4, 0.08)") // 比水平线略粗/亮一点
          .attr("stroke-width", 1)
          .style("opacity", 0) // 初始隐藏
          .transition().duration(transitionDuration).style("opacity", 1); // 平滑淡入
      });

      const buffer = 10;
      const pointScale = d3.scaleLinear()
        .domain([0, 100])
        .range([padding.left + buffer, padding.left + innerW - buffer]);

      // 计算目标像素 x+y
      data.forEach(d => {
        const xScaleForPoints = d3.scaleLinear().domain([0,100]).range([padding.left, padding.left + innerW]);
        d.__x = xScaleForPoints(d["风险值"] != null ? d["风险值"] : 0);
        const rv = d["风险值"] != null ? +d["风险值"] : 0;
        if (rv < 20) {
...
      .attr("transform", `translate(${padding.left},${innerH + padding.top + 10})`)
      .transition().duration(transitionDuration)
      .style("opacity", 1)
      .call(bottomAxis);
    
...
      .style("opacity", 0)
      .attr("fill", d => getColor(d.data, colorby))
      .attr("cx", currentWidth/2 - padding.left) // 从画布中心入场
      .attr("cy", currentHeight/2 - padding.top)
      .on("mouseover", function(evt, d){
